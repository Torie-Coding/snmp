...
"""Ansible module for SNMP subtree walking."""
...
options:
  ...
  oids:
    description:
      - List of root OIDs to walk.
      - Must be numeric dotted notation (e.g. C(1.3.6.1.2.1.2.2)).
    type: list
    elements: str
    required: true
  max_repetitions:
    description:
      - GETBULK maxRepetitions value. Controls how many rows the agent
        returns per request.
    type: int
    default: 25
  max_results:
    description:
      - Safety cap on the total number of results returned.
      - Set to C(0) for unlimited (default).
      - Use this to prevent runaway walks on large tables.
    type: int
    default: 0
author:
...
- name: Walk with a safety cap
  torie_coding.snmp.snmp_walk:
    ...
    max_repetitions: 50
    max_results: 500
  register: walk_capped

- name: Walk via SNMPv3
  torie_coding.snmp.snmp_walk:
...
results:
  description: >-
    List of all OID/value pairs found during the walk.
...
snmp_value:
  ...
  type: dict
result_count:
  description: Total number of OID/value pairs returned.
  returned: always
  type: int
"""
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            max_repetitions=dict(type="int", default=25),
            max_results=dict(type="int", default=0),
        ),
        required_if=[
    ...
    try:
        ...
        try:
            rows, error = run_walk(
                ...
                module.params["oids"],
                max_repetitions=module.params["max_repetitions"],
        ...
        if error:
            module.fail_json(msg="SNMP WALK failed: {0}".format(error))
